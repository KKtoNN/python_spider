...
if __name__ == "__main__":
        # main()
        # cmdline.execute("scrapy crawl zgjrxww".split())
    # cmdline.execute("scrapy crawl zgjrxww --nolog".split())
# Popen("scrapy crawl dizhen".split())
    # 直接循环执行
    while True:
        start_time = time.time()
        print("START TIME :{}".format(start_time))
        os.system("scrapy crawl zgjrxww --nolog")
        # os.system("scrapy crawl zgjrxww ")
        # os.system("scrapy crawl dizhen")
        end_time = time.time()
        print("政府金融服务 END TIME :{}".format(end_time))
