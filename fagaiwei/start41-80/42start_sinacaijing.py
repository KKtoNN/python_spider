...
if __name__ == "__main__":
    # main()
    # cmdline.execute("scrapy crawl sinacaijing".split())
    # cmdline.execute("scrapy crawl sinacaijing --nolog".split())
    # Popen("scrapy crawl dizhen".split())
    # 直接循环执行
    while True:
        start_time = time.time()
        print("START TIME :{}".format(start_time))
        os.system("scrapy crawl sinacaijing --nolog")
        # os.system("scrapy crawl sinacaijing ")
        # os.system("scrapy crawl dizhen")
        end_time = time.time()
        print("新浪财经  END TIME :{}".format(end_time))
