...
if __name__ == "__main__":
    ...
    # cmdline.execute("scrapy crawl shanghaizhengsuo_sipder --nolog".split())
# Popen("scrapy crawl dizhen".split())
    # 直接循环执行
    while True:
        start_time = time.time()
        print("START TIME :{}".format(start_time))
        os.system("scrapy crawl zgshangshi_spider --nolog")
        # os.system("scrapy crawl zgshangshi_spider ")
        # os.system("scrapy crawl dizhen")
        end_time = time.time()
        print("END TIME :{}".format(end_time))
        print("中国上市公司 USE TIME {}".format(end_time - start_time))
        print("------------WAIT--------------")
        time.sleep(5)
